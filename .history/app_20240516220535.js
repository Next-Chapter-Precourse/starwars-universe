...
async function showModal(item) {
  ...
  if (item.starships) {
    ...
    item.starships = starships.map(starship => starship.name);
  }

  // Convert created timestamp to human-readable format
  const createdDate = new Date(item.created);
  item.created = createdDate.toLocaleString();

  // Convert edited timestamp to human-readable format
  const editedDate = new Date(item.edited);
  item.edited = editedDate.toLocaleString();
